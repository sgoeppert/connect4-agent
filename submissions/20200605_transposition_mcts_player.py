    ...
    def descend_tree(self, c_p=0.8):
        ...
        while not n.state.is_terminal():
            if not n.is_expanded():
                ...
                is_tp = False
                if hash(next_state) in self.transpositions:
                    next_node = self.transpositions[hash(next_state)]
                    is_tp = True
                else:
                    next_node = Node(next_state)
                    self.transpositions[hash(next_state)] = next_node

                n.children[next_move] = next_node
                path.append(next_node)

                # if this was a transposition, just advance to the node
                if is_tp:
                    n = next_node
                else:
                    # if it wasn't a transposition, evaluate this node
                    return path
