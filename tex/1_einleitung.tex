...
\change[inline]{Die Einleitung muss noch komplett überarbeitet werden.}
\improvement[inline]{Motivation: warum beschäftigen Sie sich mit "Vier Gewinnt"? Vielleicht liesse sich sowas sagen: Im Grunde ist das Spielen doch ein Modell für das optimierende Entscheiden in komplexen Situationen und die dort erprobten und entwickelten Algorithmen möglicherweise Bausteine für Systeme, die in einer Vielzahl von Realweltsituationen erfolgreich Entscheidungen treffen können - also will man die dort verwendeten Techniken so gut wie möglich verstehen, um besseres Entscheiden zu lernen?}
...
Vier Gewinnt\change{Schreibweise von "Vier gewinnt" vereinheitlichen} gehört zur Gruppe der kombinatorischen Spiele. Kombinatorische Spiele zeichnen sich dadurch aus, dass sie deterministisch sind, es keine verborgenen Informationen gibt, abwechselnd gezogen wird und das Spiel nach einer endlichen Anzahl an Zügen zu Ende ist\autocite{KombinatorischeSpieltheorie2019}. Als Zufalls-freies Spiel mit perfekter Information ist "Vier Gewinnt"\change{Schreibweise von "Vier gewinnt" vereinheitlichen} ein lösbares Spiel und wurde bereits 1988 von Victor Allis\autocite{allisKnowledgeBasedApproachConnectFour1988} und unabhängig davon im selben Jahr von James D. Allen schwach gelöst\autocite{allenExpertPlayConnectFour}. Ein Spiel gilt als schwach oder stark gelöst, wenn ein realisierbarer Algorithmus existiert, mit dem für jede Startposition, bei perfektem Spiel, eine optimale Spielweise bestimmt werden kann (schwach) oder wenn in jedem Spielzustand, auch solchen die nur durch fehlerhaftes Spiel erreicht werden, der optimale Zug bestimmt werden kann (stark)\autocite{GeloesteSpiele2019}. Victor Allis hat mithilfe eines Computerprogramms "VICTOR" gezeigt, dass der erste Spieler bei beidseitig perfektem Spiel immer gewinnt, wenn er den ersten Stein in die mittlere Spalte setzt, das Spiel mindestens unentschieden endet, wenn er in die Spalten direkt daneben setzt, und verliert, wenn er in einer der anderen Spalten beginnt.

\improvement[inline]{Es wurde bis dahin noch nicht präzise eingeführt (gar nicht, wenn ich es recht sehe), wie die Zustände genau aussehen im "Vier gewinnt", wie Züge bzw. Halbzüge und Zugfolgen angegeben werden (z.B., das die Position über den Spieler entscheidet, Odd = Max, Even = Min; oder umgekehrt, wenn man ab 0 zählt), usw. Das sollte schon erläutert sein, z.B. in der Vorstellung von "Vier gewinnt" im Kapitel 1 (vielleicht zeigt man dort Abb. 4 bereits, oder etwas ähnliches, und erläutert daran die Notation im Text und, in Teilen, in der Bildunterschrift). Die Präzisierung kann man auch gut benutzen, um GENAU klar zu machen, was ein Ply oder Half-Ply nun konkret im "Vier gewinnt"-Kontext sein soll usw., das macht 1.2 eher konkreter und damit besser. Und ja, das kann durchaus schon in Kapitel 1 erläutert werden, es geht ja schließlich um das Spielen dieses Spiels ;)}

\subsection{Definition eines Agenten}

Das Ziel dieser Bachelorarbeit ist es, einen Agenten für das Spiel Vier Gewinnt\change{Schreibweise von "Vier gewinnt" vereinheitlichen} zu entwickeln. Wikipedia definiert einen Agenten wie folgt (Hervorhebungen meine):
